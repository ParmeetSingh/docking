...
with open('yolo_mobilenet.cfg','rb') as f:
    cfg = pickle.load(f)
    model = model_from_json(cfg)
    

with open('yolo_mobilenet_weights.hd5','rb') as f:
...
while success:
    ...
    try:
        ...
        if contour5_flag==True:
            ...
            text= "distance,heading,elevation in cm:"+str(np.linalg.norm(translation_vector)/100)+","+str(round(heading,1))+","+str(round(elevation,1))
